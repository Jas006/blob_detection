    ...
    def continuer(self):
        for i in range(nombre_de_canaux):
            if self.liste[i].get()==1:
                self.liste_canal.append(i)
        self.sigma_gauss=self.sigma_gauss.get()
        self.wth = self.wth.get()
        self.seuil = self.seuil.get()
        self.max = self.max.get()
        self.num = self.num.get()
        self.thresh = self.thresh.get()
        self.distance=self.distance.get()
        return self.sigma_gauss,self.wth,self.liste_canal,self.seuil,self.max,self.num,self.thresh,self.liste_canal
