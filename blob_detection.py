...
def complet_czi(path,channel,sigma_filtre_gaussien=3,taille_tophat=30,seuil_percentile=99.2,max_sigma=30, num_sigma=10, threshold=0.1, Path_enregistrement=r"C:\Users\Client\Desktop\analyse",d_semblables=3):
    ...
    z_image=len(image_importe[0,0,0,0])
    for z in range(z_image):
        for c in range(len(channel)):
            liste_excel.append(workbook.add_worksheet(f"Canal{c+1}_z{z+1}"))
        liste_excel.append(workbook.add_worksheet(f"Points semblables z{z+1}"))
    ...
    def top_hat(image_a_analyser):
        image_a_analyser = hat(image_a_analyser, taille_tophat)
        return image_a_analyser
    def thresholding(image_a_analyser):
    ...
    def blob_detection(image,gamma,zeta,somme_excel,liste):
        ...
        plt.savefig(Path_enregistrement+rf"\z{zeta+1}_channel{gamma+1}")
        somme_excel+=1
    ...
    for z in range(z_image):
        ...
        for c in range(len(channel)):
            image=image_importe[0,0,c,0,z,:,:,0]
            image_denoising=denoising(image)
            image_hdome=h_dome(image_denoising)
            image_top_hat=top_hat(image_hdome)
            image_threshold=thresholding(image_top_hat)
            liste_filtre.append(image_threshold)
        for c in range(len(channel)+1):
            if (sommep+1)%4!=0:
                blob_detection(liste_filtre[c],gamma=c,zeta=z,somme_excel=sommep,liste=liste_points)
                sommep+=1
            else:
    ...
    for z in range(z_image):
        for c in range(len(channel)):
            ...
            for i in range(len(liste_gauss_x[sommec])):
                sommeg+=1
                data, min_x, min_y = reduire_image(image_importe[0, 0, c, 0, z, :, :, 0], liste_gauss_x[sommec][i],
